...
function normalizeFilms(rawFilms) {
  return rawFilms.map(f => {
    ...
    return {
      id: f.id, title: f.title, rating: f.rating || "TBC", runtime: f.runtime || 90,
      genre: f.genre || "Other", color: f.color || "#78909c", accent: f.accent || "#b0bec5",
      film_url: f.film_url || null, poster_url: f.poster_url || null,
      showtimes, bookingUrls, screens,
...
const themes = {
  dark: {
    ...
    surface: "#0c0c14",
    surfaceAlt: "#0a0a11",
    rowEven: "rgba(255,255,255,0.008)",
    rowOdd: "transparent",
    border: "#16161f",
    borderLight: "#1e1e2a",
    text: "#e8e4dc",
    ...
    grainOpacity: 0.018,
    spotlightColor: "rgba(212,160,83,0.04)",
    barBookBg: "rgba(255,255,255,0.12)",
    barBookBorder: "rgba(255,255,255,0.18)",
    barBookHover: "rgba(255,255,255,0.35)",
    barText: "#fff",
    barSubText: "rgba(255,255,255,0.65)",
    gridStickyBg1: "#0d0c12",
    gridStickyBg2: "#0a0a10",
    gridDashColor: "#16161f",
    cardBorder: (c) => `${c}28`,
    cardBg: (c) => `linear-gradient(135deg, ${c}08 0%, transparent 100%)`,
    mobileSideBg: "#06060b",
    mobileConnector: (a) => `${a}18`,
    ccBg: "rgba(255,255,255,0.05)",
    ccBorder: "rgba(255,255,255,0.08)",
    stickyShadow: "4px 0 12px rgba(0,0,0,0.6)",
    gridCellBorder: "#06060b",
    pillBgAlpha: "15",
    pillBorderAlpha: "30",
  },
  light: {
    ...
    accent: "#a07430",
    accentGlow: "#a0743044",
    accentSoft: "rgba(160,116,48,0.07)",
    accentMed: "rgba(160,116,48,0.13)",
    headerBg: "linear-gradient(180deg, #faf8f4 0%, #f4f1ec 100%)",
    grainOpacity: 0.012,
    spotlightColor: "rgba(160,116,48,0.035)",
    barBookBg: "rgba(255,255,255,0.35)",
    barBookBorder: "rgba(255,255,255,0.5)",
    barBookHover: "rgba(255,255,255,0.6)",
    barText: "#fff",
    barSubText: "rgba(255,255,255,0.75)",
    gridStickyBg1: "#faf8f5",
    gridStickyBg2: "#f6f3ee",
    gridDashColor: "#e0dbd3",
    cardBorder: (c) => `${c}30`,
    cardBg: (c) => `linear-gradient(135deg, ${c}0c 0%, transparent 100%)`,
    mobileSideBg: "#f4f1ec",
    mobileConnector: (a) => `${a}22`,
    ccBg: "rgba(0,0,0,0.04)",
    ccBorder: "rgba(0,0,0,0.08)",
    stickyShadow: "4px 0 12px rgba(0,0,0,0.06)",
    gridCellBorder: "#e0dbd3",
    pillBgAlpha: "18",
    pillBorderAlpha: "40",
  },
};
...
export default function App() {
  ...
  const weeks = useMemo(() => {
    ...
    allDates.forEach(d => {
      ...
      if (!seen.has(monStr)) {
        ...
        wks.push({
          monStr, monDate: mon, sunDate: sun,
  ...
  const dayFilms = useMemo(() => {
    return films.filter(f => f.showtimes[selDate]).map(f => ({
      ...
      sessions: f.showtimes[selDate].map(t => ({
        time: t, startMin: timeToMin(t), adsEnd: timeToMin(t) + ADS_MIN,
  ...
  const grainOverlay = (
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, zIndex:0, pointerEvents:"none" }}>
      <div style={{
        ...
        width:"120%", height:"50%",
        background:`radial-gradient(ellipse, ${T.spotlightColor} 0%, transparent 70%)`,
      }} />
      <div style={{
        position:"absolute", inset:0, opacity:T.grainOpacity,
    ...
    return (
      <div style={{ fontFamily:T.sans, background:T.bg, color:T.text, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        <div style={{ textAlign:"center" }}>
          <div style={{ fontSize:28, fontWeight:800, color:T.accent, marginBottom:12, fontFamily:T.serif }}>Peckhamplex</div>
      ...
      <div style={{ fontFamily:T.sans, background:T.bg, color:T.text, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        <div style={{ textAlign:"center", maxWidth:400, padding:20 }}>
          ...
          <button onClick={() => window.location.reload()} style={{
            marginTop:20, padding:"10px 28px", background:T.accent, color: isDark ? T.bg : "#fff",
            border:"none", borderRadius:6, cursor:"pointer", fontFamily:T.sans, fontWeight:700, fontSize:13,
          }}>Retry</button>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ fontFamily:T.sans, background:T.bg, color:T.text, minHeight:"100vh", position:"relative", transition:"background 0.35s, color 0.35s" }}>
      {fontLink}
      {grainOverlay}
      ...
      {/* ═══════ HEADER ═══════ */}
      <div style={{ background:T.headerBg, padding:"28px 24px 20px", borderBottom:`1px solid ${T.accent}33` }}>
        <div style={{ maxWidth:1000, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
            <div>
              <div style={{ fontSize:9, letterSpacing:4, textTransform:"uppercase", color:T.accent, fontFamily:T.mono, fontWeight:700, marginBottom:4, opacity:0.7 }}>Now Showing</div>
              <h1 style={{
                fontFamily:T.serif, fontSize:32, fontWeight:900, margin:0, letterSpacing:"-0.5px", lineHeight:1,
                background: isDark
                  ? `linear-gradient(135deg, #f0ece4 0%, ${T.accent} 150%)`
                  : `linear-gradient(135deg, #1a1814 0%, ${T.accent} 150%)`,
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
              <div style={{
                ...
                <span style={{ fontSize:13, color:T.accent, fontWeight:700, fontFamily:T.mono }}>£7.59</span>
              ...
              <a href="https://www.peckhamplex.london" target="_blank" rel="noopener" style={{
                color:T.textMuted, textDecoration:"none", fontSize:11, fontFamily:T.mono, letterSpacing:0.5,
              }}>95a Rye Lane ↗</a>
              <ThemeToggle />
            </div>
          </div>
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"20px 20px 40px" }}>
          ...
          {view === "day" ? (
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <button onClick={() => goDay(-1)} disabled={!canPrevDay} className="view-btn" style={{
                ...
                color:canPrevDay?T.text:T.textFaint,
                fontFamily:T.serif, fontWeight:600, transition:"all 0.25s", lineHeight:1,
              }}>‹</button>
              <div style={{ textAlign:"center", minWidth:140 }}>
                <div style={{ fontSize:17, fontWeight:700, color:T.text, fontFamily:T.serif, letterSpacing:"-0.3px" }}>
                  {selDayInfo.day} {selDayInfo.num} {selDayInfo.mon}
                </div>
                <div style={{ fontSize:10, color:T.textDim, fontFamily:T.mono, marginTop:2 }}>
              ...
              <button onClick={() => goDay(1)} disabled={!canNextDay} className="view-btn" style={{
                ...
                border:`1.5px solid ${T.border}`, background:"transparent",
                color:canNextDay?T.text:T.textFaint,
                fontFamily:T.serif, fontWeight:600, transition:"all 0.25s", lineHeight:1,
              }}>›</button>
            </div>
          ) : selWeek ? (
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <button onClick={() => goWeek(-1)} disabled={!canPrevWeek} className="view-btn" style={{
                ...
                border:`1.5px solid ${T.border}`, background:"transparent",
                color:canPrevWeek?T.text:T.textFaint,
                fontFamily:T.serif, fontWeight:600, transition:"all 0.25s", lineHeight:1,
              }}>‹</button>
              ...
              <button onClick={() => goWeek(1)} disabled={!canNextWeek} className="view-btn" style={{
                ...
                border:`1.5px solid ${T.border}`, background:"transparent",
                color:canNextWeek?T.text:T.textFaint,
                fontFamily:T.serif, fontWeight:600, transition:"all 0.25s", lineHeight:1,
              }}>›</button>
            </div>
          ) : null}
            ...
            {dayFilms.length === 0 ? (
              ...
              (() => {
                ...
                allSessions.forEach(sess => {
                  ...
                  if (last && last.time === sess.time) last.sessions.push(sess);
                  else groups.push({ time: sess.time, startMin: sess.startMin, sessions: [sess] });
                });
                      ...
                      return (
                        <div key={group.time + gi} style={{ display:"flex", gap:0, opacity: isPast ? 0.35 : 1, transition:"opacity 0.3s" }}>
                          <div style={{ width:56, flexShrink:0, display:"flex", flexDirection:"column", alignItems:"center", position:"relative" }}>
                            <div style={{
                              fontSize:13, fontWeight:700, color: isPast ? T.textFaint : T.accent,
                              fontFamily:T.mono, padding:"4px 0", zIndex:2, background:T.bg,
                            }}>{group.time}</div>
                            {gi < groups.length - 1 && (
                              <div style={{ width:1, flex:1, background:T.mobileConnector(T.accent), minHeight:8 }} />
                            )}
                          </div>
                                ...
                                <div key={`${film.id}-${si}`} className="tkt-card" style={{
                                  ...
                                  <div style={{ flex:1, padding:"11px 14px" }}>
                                    <div style={{ fontSize:14, fontWeight:700, color:T.text, lineHeight:1.25, fontFamily:T.serif }}>
                                      {film.film_url ? (
                                        <a href={film.film_url} target="_blank" rel="noopener" style={{ color:T.text, textDecoration:"none" }}>{film.title}</a>
                                      ) : film.title}
                                    </div>
                                    <div style={{ display:"flex", gap:6, marginTop:5, alignItems:"center", flexWrap:"wrap" }}>
                                      <span style={{
                                        fontSize:9, padding:"2px 6px", borderRadius:3, fontWeight:700,
                                        background:rBg[film.rating]||"#444", color:"#fff", fontFamily:T.mono, letterSpacing:0.5,
                                      }}>{film.rating}</span>
                ...
                <div style={{ marginLeft:180, position:"relative", height:30, marginBottom:4 }}>
                  {hourMarks.map(m => (
                    <div key={m} style={{
                      ...
                      fontSize:10, fontFamily:T.mono, color:T.textDim, fontWeight:400, letterSpacing:0.5,
                    }}>{minToTime(m)}</div>
                  ))}
                </div>
                ...
                <div ref={tlRef} style={{ position:"relative" }}>
                  {dayFilms.map((film, fi) => {
                    ...
                    <div key={film.id} style={{
                      ...
                      background: fi % 2 === 0 ? T.rowEven : T.rowOdd,
                      borderRadius:8, overflow:"visible",
                      /* Row grows when a bar inside is hovered */
                      minHeight: anyHov ? 74 : 56,
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                          ...
                          <div>
                            <div style={{ fontSize:12, fontWeight:700, color:T.textSub, lineHeight:1.2, fontFamily:T.serif }}>
                              {film.film_url ? (
                                <a href={film.film_url} target="_blank" rel="noopener" style={{ color:T.textSub, textDecoration:"none" }}>{film.title}</a>
                              ) : film.title}
                            </div>
                      ...
                      <div style={{ flex:1, position:"relative", padding:"6px 0" }}>
                        {hourMarks.map(m => (
                          <div key={m} style={{
                            ...
                            width:1, background: isDark ? `${T.accent}0a` : `${T.accent}12`, zIndex:0,
                          }} />
                        ))}
                        {halfMarks.map(m => (
                          <div key={m} style={{
                            position:"absolute", left:`${pct(m)}%`, top:0, bottom:0,
                            width:1, background: isDark ? `${T.accent}05` : `${T.accent}08`, zIndex:0,
                          }} />
                        ))}
                          ...
                          return (
                            <div key={si}
                              ...
                              style={{
                                ...
                                boxShadow: isHov
                                  ? `0 8px 32px ${film.color}44, 0 0 0 1px ${film.color}55, inset 0 1px 0 rgba(255,255,255,0.06)`
                                  : isDark
                              ...
                              <div style={{
                                ...
                                <div style={{ flex:1, minWidth:0 }}>
                                  <div style={{
                                    fontSize:10.5, fontWeight:700, color:T.barText,
                                  ...
                                  {isHov && (
                                    <div style={{ fontSize:9, color:T.barSubText, marginTop:3, fontFamily:T.mono }}>
                                ...
                                {sess.bookingUrl && (
                                  <a href={sess.bookingUrl} target="_blank" rel="noopener"
                                    ...
                                    className="book-btn" title="Book tickets"
                                    style={{
                                      display:"flex", alignItems:"center", justifyContent:"center",
                                      padding:"4px 8px", borderRadius:4,
                                      background:T.barBookBg, border:`1px solid ${T.barBookBorder}`,
                  ...
                  if(nowMin >= axisStart && nowMin <= axisEnd){
                    return (
                      <div style={{
                        ...
                        top:30, bottom:0, width:2, background:T.accent, zIndex:20,
                        boxShadow:`0 0 12px ${T.accentGlow}`,
                      }}>
                        <div style={{
                          position:"absolute", top:-8, left:-14,
                          fontSize:8, fontWeight:700, color:T.accent,
                          background:T.bg, padding:"1px 5px", borderRadius:3,
            ...
            <div style={{ overflowX:"auto", borderRadius:8, border:`1px solid ${T.border}` }}>
              <table style={{ width:"100%", borderCollapse:"separate", borderSpacing:0, minWidth:400 }}>
                <thead>
                  <tr>
                    <th style={{
                      padding:"12px 14px", textAlign:"left", fontSize:10, color:T.textDim, fontWeight:600,
                      borderBottom:`1px solid ${T.border}`, position:"sticky", left:0,
                      background:T.surface, zIndex:5, minWidth:150,
                      boxShadow:T.stickyShadow,
                    ...
                    {(selWeek ? selWeek.dates : allDates).map(d => {
                      ...
                      return (
                        <th key={d} onClick={() => { setSelDate(d); setView("day"); }} style={{
                          ...
                          color: isT ? T.accent : T.textDim,
                          background: isT ? T.accentSoft : "transparent",
                          borderBottomColor: isT ? `${T.accent}55` : T.border,
                          minWidth:72, transition:"color 0.2s", fontFamily:T.mono,
                        }}>
                ...
                <tbody>
                  {(() => { const weekDates = selWeek ? selWeek.dates : allDates; return films.filter(f => weekDates.some(d => f.showtimes[d])).map((film, fi) => (
                    <tr key={film.id} style={{ background: fi%2===0 ? T.rowEven : T.rowOdd }}>
                      <td style={{
                        padding:"10px 12px", borderBottom:`1px solid ${T.gridCellBorder}`,
                        position:"sticky", left:0,
                        background: fi%2===0 ? T.gridStickyBg1 : T.gridStickyBg2,
                        zIndex:4, boxShadow:T.stickyShadow,
                      }}>
                        <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                          <div style={{ width:3, height:24, borderRadius:1.5, background:`linear-gradient(180deg, ${film.color}, ${film.color}44)` }} />
                          <div>
                            <div style={{ fontSize:11, fontWeight:700, color:T.textSub, fontFamily:T.serif }}>
                              {film.film_url ? (
                                <a href={film.film_url} target="_blank" rel="noopener" style={{ color:T.textSub, textDecoration:"none" }}>{film.title}</a>
                              ) : film.title}
                            </div>
                              ...
                              <div style={{ display:"flex", flexDirection:"column", gap:5, alignItems:"center" }}>
                                {times.map((t,i) => {
                                  ...
                                  const pill = (
                                    ...
                                    title={film.screens?.[d]?.[t] ? `${film.screens[d][t]}${isHoh ? " · HoH" : ""}` : (isHoh ? "Hard of Hearing" : "")}
                                    >
                                      {t}{isHoh ? " CC" : ""}
                                    </span>
                                  );
        ...
        <div style={{
          ...
          background: isDark
            ? "linear-gradient(135deg, rgba(212,160,83,0.02) 0%, transparent 100%)"
            : "linear-gradient(135deg, rgba(160,116,48,0.03) 0%, transparent 100%)",
          ...
          <div style={{ display:"flex", gap:14, flexWrap:"wrap", fontSize:10, color:T.textDim, fontFamily:T.mono, marginTop: isMobile ? 0 : undefined }}>
            ...
            <span style={{ display:"flex", alignItems:"center", gap:4 }}>
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={T.textMuted} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/>
                <path d="M13 5v2"/><path d="M13 17v2"/><path d="M13 11v2"/>
              </svg>
              = Book tickets
          ...
          {scrapedAt && (
            <div style={{
              fontSize:9, color:T.textFaint, marginTop:8, fontFamily:T.mono,
              paddingTop:8, borderTop:`1px solid ${T.border}`, letterSpacing:0.3,
            }}>
